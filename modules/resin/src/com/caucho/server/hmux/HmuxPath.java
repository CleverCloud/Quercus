...
public class HmuxPath extends FilesystemPath {
           ...
           String path, String query)
  {
    ...
    _host = ((HmuxPath) root)._host;
    _port = ((HmuxPath) root)._port;
    _query = query;
                            ...
                            int offset)
  {
    ...
    HmuxPath root = create(host, port);
  ...
  protected HmuxPath create(String host, int port)
  {
    return new HmuxPath(host, port);
  }
  ...
  protected CacheEntry getCache()
  {
    ...
    synchronized (_cacheEntry) {
      try {
        ...
        HmuxStreamWrapper stream = (HmuxStreamWrapper) openReadImpl();
        stream.setHead(true);
  ...
  {
    if (! (o instanceof HmuxPath))
      return false;

    HmuxPath test = (HmuxPath) o;
