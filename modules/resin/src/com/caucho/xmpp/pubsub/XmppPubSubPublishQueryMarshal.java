...
public class XmppPubSubPublishQueryMarshal extends AbstractXmppMarshal {
  private static final Logger log
    = Logger.getLogger(XmppPubSubPublishQueryMarshal.class.getName());
  ...
  {
    PubSubPublishQuery publish = (PubSubPublishQuery) object;

    out.writeStartElement("", "pubsub", getNamespaceURI());
    out.writeNamespace("", getNamespaceURI());

    out.writeStartElement("publish");

    out.writeAttribute("node", publish.getNode());

    PubSubItem item = publish.getItem();
